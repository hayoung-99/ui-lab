...
import { DEFAULT_ADJUST, LiquidMetalButton, type RimAdjust } from './LiquidMetalButton/LiquidMetalButton';
...
const KEYWORDS: { group: string; items: Keyword[] }[] = [
  ...
  },
  {
    ...
    items: [
      '750×220 · 60fps',
      '640 프레임',
      { code: 'rim-reference.png' },
      '512×594 · 164KB',
      `${REFERENCE_PERIOD.toFixed(2)}s 주기`,
    ],
  },
  {
    group: 'Behavior',
  ...
  },
  {
    ...
    items: [
      ...
      'Vite 8',
      'Tailwind CSS 4',
      '페이지 청크 14KB · gzip 6KB',
      { code: 'frames.swift' },
      { code: 'build_rim_texture.py' },
    ],
  },
];
  ...
  return (
    <section className={`min-w-0 flex-1 ${cardClass}`}>
      <header className="mb-4">
        <h2 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100">Spec</h2>
      </header>
      <dl className="grid grid-cols-2 gap-x-6 gap-y-5 sm:grid-cols-4">
        {KEYWORDS.map(({ group, items }) => (
          <div key={group} className="flex flex-col gap-2">
            <dt className="text-[11px] font-semibold tracking-widest text-neutral-500 uppercase dark:text-neutral-400">
              {group}
            </dt>
      ...
      <div className="mb-1 flex items-baseline justify-between text-[13px]">
        ...
        <span
          className={`font-mono text-[12px] tabular-nums ${
            changed ? 'text-neutral-900 dark:text-neutral-100' : 'text-neutral-400'
          }`}
        >
          {value}
          {unit}
...
function ControlPanel({ value, onChange }: { value: Controls; onChange: (next: Controls) => void }) {
  const setAdjust = (patch: Partial<RimAdjust>) => onChange({ ...value, adjust: { ...value.adjust, ...patch } });
    ...
    <section className={`w-[360px] ${cardClass}`}>
      ...
      <div className="space-y-4">
        <Slider
          ...
          unit="px"
          defaultValue={7}
          onChange={(ring) => onChange({ ...value, ring })}
        />
        <Slider
          label="재생 속도"
          value={value.rate}
    ...
    <main className="flex flex-col gap-8">
      ...
      <header className="flex flex-wrap items-baseline gap-3">
        ...
        <p className="flex flex-wrap items-baseline gap-x-2 text-xs text-neutral-500 dark:text-neutral-400">
          ...
          {meta.updatedAt !== meta.createdAt && (
            <span>
              · 수정{' '}
              <time dateTime={meta.updatedAt} className="font-mono">
                {meta.updatedAt}
              </time>
            </span>
          )}
      ...
      <section className="flex flex-col gap-8 lg:flex-row lg:items-stretch">
        <div className="flex min-h-[380px] flex-1 items-center justify-center rounded-[64px] border border-white/50 bg-[#dcdcde] bg-[radial-gradient(ellipse_55%_50%_at_50%_50%,rgba(255,255,255,0.45),transparent)] p-12 shadow-[inset_0_2px_10px_rgba(0,0,0,0.08),0_1px_0_rgba(255,255,255,0.6)] transition-colors lg:min-h-[460px] dark:border-white/5 dark:bg-[#1d1d21] dark:bg-[radial-gradient(ellipse_55%_50%_at_50%_50%,rgba(255,255,255,0.05),transparent)] dark:shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]">
          <LiquidMetalButton theme={theme} ring={controls.ring} rate={controls.rate} adjust={controls.adjust}>
            Elements
          </LiquidMetalButton>
        </div>
